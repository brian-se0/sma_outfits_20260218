...
ALPACA_BASIC_HISTORICAL_START ?= 2016-01-01T00:00:00Z
ALPACA_BASIC_HISTORICAL_DELAY_MINUTES ?= 15
...
PROVEN_EDGE_CONFIG ?= $(if $(strip $(VALIDATION_CONFIG_OVERRIDE)),$(VALIDATION_CONFIG_OVERRIDE),$(CONFIG))
REPLICATION_CONFIG ?= configs/settings.jan2025_confluence_atr_svix211_106_crossctx_replication_v1.yaml
REPLICATION_SYMBOLS ?= QQQ,SPY,TQQQ,SQQQ,SVIX,VIXY,XLF,SMH,SOXL
...
prove-edge:
	$(MAKE) e2e CONFIG=$(PROVEN_EDGE_CONFIG) PROFILE=$(PROFILE) START=$(START) END=$(END) ANALYSIS_START=$(ANALYSIS_START) ANALYSIS_END=$(ANALYSIS_END) WARMUP_DAYS=$(WARMUP_DAYS) SYMBOLS=$(PROVEN_EDGE_SYMBOLS) TIMEFRAMES=$(PROVEN_EDGE_TIMEFRAMES) BACKFILL_SYMBOLS=$(PROVEN_EDGE_SYMBOLS) BACKFILL_TIMEFRAMES=$(PROVEN_EDGE_TIMEFRAMES) REPLAY_SYMBOLS=$(PROVEN_EDGE_SYMBOLS) REPLAY_TIMEFRAMES=$(PROVEN_EDGE_TIMEFRAMES) STAGES=validate-config,backfill,replay,report
	$(MAKE) verify-readiness CONFIG=$(PROVEN_EDGE_CONFIG) START=$(ANALYSIS_START) END=$(ANALYSIS_END) SYMBOLS=$(PROVEN_EDGE_SYMBOLS) TIMEFRAMES=$(PROVEN_EDGE_TIMEFRAMES) VERIFY_READINESS_ARGS=--require-academic-validation

prove-edge-replication:
	$(MAKE) discover-range CONFIG=$(REPLICATION_CONFIG) SYMBOLS=$(REPLICATION_SYMBOLS) TIMEFRAMES=$(REPLICATION_TIMEFRAMES) DISCOVER_RANGE_OUTPUT=$(REPLICATION_DISCOVER_OUTPUT) READINESS_END=$(REPLICATION_END)
	powershell -NoProfile -Command "if ([string]::IsNullOrWhiteSpace('$(REPLICATION_FULL_RANGE_START)')) { throw 'Replication discover-range manifest missing full_range_start in $(REPLICATION_DISCOVER_OUTPUT)' }"
	$(MAKE) e2e CONFIG=$(REPLICATION_CONFIG) PROFILE=custom START=$(REPLICATION_FULL_RANGE_START) END=$(REPLICATION_END) ANALYSIS_START=$(REPLICATION_FULL_RANGE_START) ANALYSIS_END=$(REPLICATION_END) WARMUP_DAYS=$(WARMUP_DAYS) SYMBOLS=$(REPLICATION_SYMBOLS) TIMEFRAMES=$(REPLICATION_TIMEFRAMES) BACKFILL_SYMBOLS=$(REPLICATION_SYMBOLS) BACKFILL_TIMEFRAMES=$(REPLICATION_TIMEFRAMES) REPLAY_SYMBOLS=$(REPLICATION_SYMBOLS) REPLAY_TIMEFRAMES=$(REPLICATION_TIMEFRAMES) STAGES=validate-config,backfill,replay,report
	$(MAKE) verify-readiness CONFIG=$(REPLICATION_CONFIG) START=$(REPLICATION_FULL_RANGE_START) END=$(REPLICATION_END) SYMBOLS=$(REPLICATION_SYMBOLS) TIMEFRAMES=$(REPLICATION_TIMEFRAMES) VERIFY_READINESS_ARGS=--require-academic-validation
